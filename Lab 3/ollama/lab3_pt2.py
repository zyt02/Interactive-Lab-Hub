...
class OllamaVoiceAssistant:
    ...
    def initialize_scores(self, initial_amount=10000, players=['user1', 'user2', 'user3']):
        ...
        player_list = ", ".join(self.player_scores.keys())
    ...
    def parse_score_command(self, user_input):
        ...
        player_names_pattern = r"user\s*1|user\s*2|user\s*3"

        patterns = [
            # word-based operations: user1 plus 300
            ...
            # symbol-based: user1 + 300, user 2 - 500
            rf"(?P<player>{player_names_pattern})\s*(?P<op>\+|\-)\s*(?P<amount>\d+)",
            # add/subtract to/from user
        ...
        for pat in patterns:
            ...
            if match:
                player = match.group("player").replace(" ", "")  # normalize: "user 1" → "user1"
    ...
    def run_conversation(self):
        ...
        print("Say 'start game' to begin tracking scores for user1, user2, and user3.")
            ...
            try:
                ...
                if self.waiting_for_score_update and self.score_initialized:
                    ...
                    if score_command:
                        ...
                        self.speak("Sorry, I still didn't catch that. Please say something like 'user one plus 300' or 'user two minus 500'.")
                        continue
                ...
                # Show scores
                if 'show scores' in user_input or 'current balance' in user_input:
                ...
                if ('update score' in user_input or 'change score' in user_input) and self.score_initialized:
                    ...
                    self.speak("Ready to update. Please say which user and how much. For example, 'user one plus 300'.")
                    continue
